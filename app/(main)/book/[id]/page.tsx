...
export default function BookReaderPage() {
    ...
    const loadBook = async () => {
      try {
        ...
        if (result.success && result.book) {
          ...
          // Use fileUrl from the server action (Local Filesystem)
          if (result.book.fileUrl) {
            setPdfUrl(result.book.fileUrl);
          } else {
            // Fallback for legacy Appwrite books (if any)
  ...
  useEffect(() => {
    return () => {
      if (bookId && currentPageRef.current > 1) {
        updateReadingProgress(bookId, currentPageRef.current).catch(
          console.error
        );
      }
    };
  ...
  useEffect(() => {
    if (!book || !totalPages || currentPage === 0 || !isInitializedRef.current)
      return;
    ...
    try {
      ...
      if (result.success) {
        ...
        if (annotationId && result.explanation) {
          const saveResult = await saveAnnotationExplanation(
            annotationId,
            result.explanation
          );
          if (saveResult.success) {
            setAnnotations((prev) =>
              prev.map((a) =>
                a.id === annotationId
                  ? { ...a, explanation: result.explanation }
                  : a
              )
            );
          }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200/60 shadow-sm">
        <div className="max-w-[1920px] mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-800 leading-none truncate max-w-sm">
                {book.title}
              </h1>
          ...
          <button
            onClick={() =>
              book &&
              setEditModal({ isOpen: true, bookId: book.id, title: book.title })
            }
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 text-sm font-bold rounded-full hover:bg-indigo-100 transition-colors"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <button
            ...
            className="ml-2 p-2 text-slate-400 hover:text-indigo-600 rounded-full transition-colors"
            title="Full Screen"
          >
            <svg
              className="w-6 h-6"
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div
          ref={fullScreenRef}
          className="flex-1 relative bg-slate-100 flex flex-col min-h-0 overflow-hidden group order-2 lg:order-1 h-[50vh] lg:h-auto"
        >
            ...
            {loading && (
              <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/90 backdrop-blur-sm">
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2 bg-indigo-50 text-indigo-600 text-sm font-bold rounded-lg hover:bg-indigo-100 transition-colors"
                >
                  Reload Book
                </button>
              </div>
              ...
              <div className="p-5 space-y-6">
                ...
                <AnimatePresence>
                  {(explaining || explanation) && (
                    <motion.div
                      ...
                      className="bg-gradient-to-br from-indigo-50 to-white rounded-xl p-4 border border-indigo-100 shadow-sm overflow-hidden"
                    >
                      ...
                      <div className="prose prose-sm prose-indigo text-slate-600 leading-relaxed text-sm">
                        {explaining ? (
                          <p className="animate-pulse">Analyzing text...</p>
                        ) : (
                ...
                {selectedText && (
                  <div className="bg-amber-50 p-4 rounded-xl border border-amber-100">
                    ...
                    <p className="text-sm text-slate-700 italic border-l-2 border-amber-300 pl-3 mb-3 line-clamp-3">
                      &quot;{selectedText}&quot;
                    </p>
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleQuickExplain(selectedText)}
                        className="flex-1 py-1.5 bg-white text-amber-700 text-xs font-bold rounded border border-amber-200 shadow-sm hover:shadow flex items-center justify-center gap-1"
                      >
                        <svg
                          className="w-3 h-3"
                          fill="none"
                      ...
                      <button
                        onClick={() => startChatAboutText(selectedText)}
                        className="flex-1 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded shadow-sm hover:bg-indigo-700 flex items-center justify-center gap-1"
                      >
                        <svg
                          className="w-3 h-3"
                          fill="none"
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-slate-400 hover:bg-slate-100 rounded"
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
                ...
                <div className="space-y-3">
                  {annotations.map((ann) => (
                    ...
                    >
                      <p className="text-sm font-medium text-slate-800 line-clamp-3 mb-3">
                        &quot;{ann.text}&quot;
                      </p>
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => handleQuickExplain(ann.text, ann.id)}
                          ...
                          <button
                            onClick={() => startChatAboutText(ann.text, ann.id)}
                          ...
                          <button
                            onClick={() => goToAnnotationPage(ann.pageNumber)}
                ...
                <div className="px-4 py-3 border-b border-slate-200 bg-white sticky top-0 z-10">
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedAnnotationId(e.target.value || null)
                      }
                ...
                <div className="flex-1 p-4 space-y-4 overflow-y-auto bg-slate-50/50">
                  {messages.length === 0 ? (
                    <div className="text-center py-16 px-6">
                      ...
                      <p className="text-slate-500 text-sm">
                        Ask questions about this book or select text to discuss
                        it.
                      </p>
                    </div>
                  ) : (
                  ...
                  {sendingMsg && (
                    <div className="flex gap-3">
                      ...
                      <div className="bg-white border border-slate-200 p-3 rounded-2xl rounded-tl-none text-slate-500 text-sm">
                        Thinking...
                      </div>
                    </div>
                  )}
